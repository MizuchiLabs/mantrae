<script lang="ts">
	...
	let config = $state({} as HttpRouter);
	...
	onMount(async () => {
		if (router.config) config = unmarshalConfig(router.config);

		// Get cert resolvers
		const response = await routerClient.listRouters({
			profileId: router.profileId ?? profile.id,
			// type: router.type, //TODO: Filter by type
			limit: -1n,
			offset: 0n
		});
		const resolverSet = new Set(
			response.routers
				.filter((r) => {
					config = unmarshalConfig(r.config);
					if (!config?.tls?.certResolver) return false;
					return true;
				})
				.map((r) => {
					config = unmarshalConfig(r.config);
					return config.tls?.certResolver ?? '';
				})
		);
			...
			<div class="flex flex-wrap gap-1">
				{#each certResolvers as resolver (resolver)}
					{#if resolver !== config.tls?.certResolver}
						<Badge
							onclick={() => config.tls && (config.tls.certResolver = resolver)}
