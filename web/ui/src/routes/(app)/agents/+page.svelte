<script lang="ts">
	...
	import { formatTs, toDate } from '$lib/utils';
	...
	let data = $state({} as Agent);
	let open = $state(false);

	const agentData = agent.list();
	const createAgent = agent.create();
	const deleteAgent = agent.delete();
	...
	const columns: ColumnDef<Agent>[] = [
		...
		},
		{
			...
			cell: ({ row }) => {
				...
				return formatTs(row.original.updatedAt, 'relative');
			}
		},
		{
			id: 'actions',
