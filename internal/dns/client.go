...
func getProvider(id int64, q *db.Queries) (DNSProvider, error) {
	...
	var d DNSProviderConfig
	config, ok := provider.Config.(string)
	if !ok {
		return nil, fmt.Errorf("invalid config type")
	}
	if err := json.Unmarshal([]byte(config), &d); err != nil {
		return nil, err
	}

	switch provider.Type {
	case "cloudflare":
		return d.NewCloudflareProvider(), nil
	case "powerdns":
		return d.NewPowerDNSProvider(), nil
	case "technitium":
		return d.NewTechnitiumProvider(), nil
	default:
