...
CREATE TABLE udp_router_dns_providers (
  udp_router_id INTEGER NOT NULL,
  dns_provider_id INTEGER NOT NULL,
  PRIMARY KEY (udp_router_id, dns_provider_id),
  FOREIGN KEY (udp_router_id) REFERENCES udp_routers (id) ON DELETE CASCADE,
  FOREIGN KEY (dns_provider_id) REFERENCES dns_providers (id) ON DELETE CASCADE
);
