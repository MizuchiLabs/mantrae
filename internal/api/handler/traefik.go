...
func scheduleUpdate(r *http.Request, app *config.App, profileID int64) {
	...
	go func() {
		_, _, _ = updateGroup.Do(instanceName, func() (any, error) {
			...
			result, err := traefik.UpdateTraefikInstance(r, app.Conn.Q, profileID)
			if err != nil {
...
func PublishTraefikConfig(a *config.App) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		profile, err := a.Conn.Q.GetProfileByName(r.Context(), r.PathValue("name"))
		if err != nil {
		...
		cfg, err := traefik.BuildDynamicConfig(r.Context(), a.Conn.Q, *profile)
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
