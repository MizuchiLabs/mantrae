...
func (s *HTTPMiddlewareOps) Get(
	...
	result, err := s.app.Conn.Query.GetHttpMiddleware(ctx, req.Id)
	if err != nil {
		return nil, err
	}
...
func (s *HTTPMiddlewareOps) Create(
	...
	if req.IsDefault {
		if err = s.app.Conn.Query.UnsetDefaultHttpMiddleware(ctx, req.ProfileId); err != nil {
			return nil, err
		}
	}

	result, err := s.app.Conn.Query.CreateHttpMiddleware(ctx, params)
	if err != nil {
		return nil, err
	}
...
func (s *HTTPMiddlewareOps) Update(
	...
	middleware, err := s.app.Conn.Query.GetHttpMiddleware(ctx, req.Id)
	if err != nil {
		return nil, err
	}

	// Make sure routers using this middleware use the new name
	routers, err := s.app.Conn.Query.
	...
	result, err := s.app.Conn.Query.UpdateHttpMiddleware(ctx, params)
	if err != nil {
		return nil, err
	}
...
func (s *HTTPMiddlewareOps) Delete(
	...
	if err := s.app.Conn.Query.DeleteHttpMiddleware(ctx, req.Id); err != nil {
		return nil, err
	}
...
func (s *TCPMiddlewareOps) Get(
	...
	result, err := s.app.Conn.Query.GetTcpMiddleware(ctx, req.Id)
	if err != nil {
		return nil, err
	}
	return &mantraev1.GetMiddlewareResponse{
...
func (s *TCPMiddlewareOps) Create(
	...
	if req.IsDefault {
		if err = s.app.Conn.Query.UnsetDefaultTcpMiddleware(ctx, req.ProfileId); err != nil {
			return nil, err
		}
	}

	result, err := s.app.Conn.Query.CreateTcpMiddleware(ctx, params)
	if err != nil {
		return nil, err
	}
...
func (s *TCPMiddlewareOps) Update(
	...
	// Get old middleware for router update
	middleware, err := s.app.Conn.Query.GetTcpMiddleware(ctx, req.Id)
	if err != nil {
		return nil, err
	}
	...
	result, err := s.app.Conn.Query.UpdateTcpMiddleware(ctx, params)
	if err != nil {
		return nil, err
	}
...
func (s *TCPMiddlewareOps) Delete(
	...
	if err := s.app.Conn.Query.DeleteTcpMiddleware(ctx, req.Id); err != nil {
		return nil, err
	}
...
func (s *TCPMiddlewareOps) List(
	...
	totalCount, err := s.app.Conn.Query.CountTcpMiddlewares(ctx, &db.CountTcpMiddlewaresParams{
		ProfileID: req.ProfileId,
		AgentID:   req.AgentId,
	})
	if err != nil {
