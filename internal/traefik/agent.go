...
func DecodeAgentConfig(DB *sql.DB, agent db.Agent) error {
	var params db.UpdateTraefikConfigParams
	params.Source = source.Agent
	params.ProfileID = agent.ProfileID

	// Initialize merged configuration
	params.Config = &db.TraefikConfiguration{
	...
	for _, container := range *agent.Containers {
		...
		for k, v := range runtimeConfig.TCPRouters {
			...
			params.Config.Services[k] = &db.ServiceInfo{
				TCPServiceInfo: &runtime.TCPServiceInfo{TCPService: &service},
			}
		}
		...
		for k, v := range runtimeConfig.UDPRouters {
			...
			params.Config.Services[k] = &db.ServiceInfo{
				UDPServiceInfo: &runtime.UDPServiceInfo{UDPService: &service},
			}
		}
	}

	q := db.New(DB)
	if err := q.UpdateTraefikConfig(context.Background(), params); err != nil {
		return err
	}
